...
import { calculateDateForDay, isToday } from "@/shared/utils/dateCalculations";
...
export const getOptimizedReadingPlanData = async (userId: string, startDate: string) => {
  ...
  if (!profileData?.selected_plan_id) {
    ...
    console.warn('Aucun plan de lecture sélectionné.');
    return [];
  }
...
export const optimizedToggleChapterStatus = async (
  ...
  if (!sessionData.session) {
    ...
    if (!silent) {
      console.warn("Vous devez être connecté pour modifier le statut de lecture");
    }
  ...
  try {
    ...
    const { ActivityService } = await import('@/features/auth/services/auth/activityService');
    ...
    if (!silent) {
      console.log(newStatus === 'completed' ? "Passage marqué comme lu" : "Passage marqué comme non lu");
    }
