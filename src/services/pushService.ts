...
class PushService {
    ...
    if (!this.vapidPublicKeyPromise) {
      this.vapidPublicKeyPromise = (async () => {
        try {
          ...
          if (error) {
            logger.warn(
              "⚠️ Impossible de récupérer la clé VAPID runtime:",
              error,
            );
            return null;
          }
  ...
  private isSubscriptionUsingVapidKey(
  ...
  async subscribe(): Promise<boolean> {
    try {
      ...
      if (subscription) {
        const usesExpectedKey = this.isSubscriptionUsingVapidKey(
          subscription,
          vapidPublicKey,
        );
        ...
        if (!usesExpectedKey) {
          logger.warn(
            "⚠️ Souscription liée à une ancienne clé VAPID, recréation…",
          );
  ...
  private async saveSubscription(
    ...
    if (deactivateError) {
      logger.warn(
        "⚠️ Impossible de désactiver les anciens devices web:",
        deactivateError,
      );
    }
