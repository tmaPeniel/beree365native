...
try {
  ...
  if (privBytes.length !== 32) {
    throw new Error(
      `VAPID private key must decode to 32 bytes (got ${privBytes.length})`,
    );
  }
...
console.log(
  "[send-push-notification] Boot. VAPID pair valid=",
  vapidPairValid,
  "pubLen=",
...
serve(async (req) => {
  ...
  if (!vapidPairValid) {
    console.error(
      "[send-push-notification] ❌ Paire VAPID invalide:",
      vapidPairError,
    );
    return new Response(
      JSON.stringify({
        ...
        derivedPublicKeyPreview: derivedPublicKey
          ? derivedPublicKey.slice(0, 12) + "…"
          : null,
        runtimePublicKeyPreview: VAPID_PUBLIC_KEY
  ...
  try {
    ...
    console.log(
      "[send-push-notification] Body brut:",
    ...
    if (!parsed.success) {
      console.error(
        "[send-push-notification] ❌ Validation échouée:",
    ...
    if (devicesError) {
      console.error(
        "[send-push-notification] ❌ Erreur fetch devices:",
        devicesError,
      );
      return new Response(
        JSON.stringify({
    ...
    console.log(
      `[send-push-notification] Devices actifs trouvés: ${devices?.length ?? 0}`,
    );
    devices?.forEach((d) => {
      console.log(
        `  • device ${d.id} user=${d.user_id} endpoint=${truncate(d.push_endpoint, 80)}`,
      );
    });
    ...
    for (const device of devices) {
      ...
      if (result.success) {
        ...
        failCount++;
        console.error(
          `[send-push-notification] ❌ Push KO device=${device.id} status=${result.statusCode} err=${result.error}`,
        );
        ...
        if (result.statusCode === 410 || result.statusCode === 404) {
          console.log(
            `[send-push-notification] Désactivation device expiré ${device.id}`,
          );
          await supabase
            .from("user_devices")
    ...
    console.log(
      `[send-push-notification] ⏹ Terminé en ${elapsed}ms — devices=${devices.length} sent=${successCount} failed=${failCount}`,
    );
