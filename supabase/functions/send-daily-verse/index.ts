...
serve(async (req) => {
  ...
  try {
    ...
    const usersToNotify = users.filter((user: any) => {
      const [prefHour, prefMinute] = user.notification_preferences.daily_verse_time.split(':').map(Number);
      let diff = Math.abs((prefHour * 60 + prefMinute) - (currentHour * 60 + currentMinute));
      if (diff > 720) diff = 1440 - diff;
      return diff <= 30;
    });

    if (usersToNotify.length === 0) {
      return new Response(JSON.stringify({ message: 'No users scheduled', count: 0 }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    for (const user of usersToNotify as any[]) {
      try {
        ...
        const payload = { title, body: message, tag: 'daily-verse', data: { type: 'daily_verse', day_number: user.current_day_number, reference: verse.reference } };
        ...
        await supabase.from('notification_logs').insert({
          user_id: user.id, notification_type: 'daily_verse', title, body: message, success: true,
        });
        successCount++;
      } catch (error: any) {
        errorCount++;
        await supabase.from('notification_logs').insert({
          user_id: user.id, notification_type: 'daily_verse', title: 'Failed', body: error.message, success: false, error_message: error.message,
        });
      }
    }
