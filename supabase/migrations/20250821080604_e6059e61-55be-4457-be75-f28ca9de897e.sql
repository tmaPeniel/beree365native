...
-- Fix: calculate_user_badges - replace invalid comparisons (integer > interval)
-- Use integer day-gap comparisons consistently.
...
BEGIN
  ...
  -- Resume after gap (default check for > 3 days) - compare integers (days)
  WITH per_day AS (
    SELECT (completed_at AT TIME ZONE 'UTC')::date AS day,
           min(completed_at) AS first_time
    FROM public.user_progress
  ...
  SELECT EXISTS(
    ...
    WHERE prev_day IS NOT NULL AND (day - prev_day) > 3
  ...
  LOOP
    ...
    ELSIF (_badge.criteria ->> 'type') = 'resume_after_gap' THEN
      ...
      BEGIN
        ...
        SELECT EXISTS(
          SELECT 1 FROM lagged 
          WHERE prev_day IS NOT NULL AND (day - prev_day) > _gap_days
        ) INTO _has_resumed;
